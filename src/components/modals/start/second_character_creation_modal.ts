...
export default class StartSubcommandCharacterInformationModal extends ModalCommand {
  ...
  public override filter(ctx: ModalContext) {
    return ctx.customId === this.characterInformationModalConstants['MODAL_ID']
  }
  ...
  public override async run(ctx: ModalContext) {
    ...
    const doneEmbed = ui.embeds.success('All done!', {
      ...
      You're all set, use the \`/economy tutorial\` command next to unlock the rest of the bot!`,
    })
